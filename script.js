...
if (playBtn) {
    playBtn.onclick = function () {
        ...
        var senderNameInput = document.getElementById("giftSenderName");
        var messageInput = document.getElementById("giftMessage");

        var musicUrl = musicUrlInput ? musicUrlInput.value.trim() : "";
        var senderName = senderNameInput ? senderNameInput.value.trim() : "";
        var customMessage = messageInput ? messageInput.value.trim() : "";

        // نحفظها بشكل عام حتى تستخدمها الشرائح الأخرى (الاسم + الرسالة)
        window.giftCustomName = senderName;
        ...
        function continueToGift() {
            ...
            var tag = document.getElementById("giftSenderTag");
            if (tag) {
                if (senderName) {
                    tag.textContent = "🎁 من: " + senderName;
